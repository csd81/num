x=[0.5, 1, 1.5, 2.5, 3];           %x values
y=[0.7, 1.1, 1.6, 2.1, 2.3];  %y values

n=length(x);
A=zeros(2);
d=[0;0];
for i=1:n
    A(1,1)=A(1,1)+log(x(i))^2;
    A(1,2)=A(1,2)+log(x(i));
    d(1)=d(1)+log(x(i))*log(y(i));
    d(2)=d(2)+log(y(i));
end
A(2,1)=A(1,2);
A(2,2)=n;
...
err2=0;
for i=1:n
    err=err+(powf(x(i),a,b)-y(i))^2;
    err2=err2+(AA*log(x(i))+BB-log(y(i)))^2;
end
...
ttl=sprintf('$%10.8f\\cdot x^{%10.8f}$',b,a);
